...
// Erlaubt Anfragen von allen Quellen. Für die Entwicklung ist das in Ordnung.
// Für die Produktion könnten Sie hier Ihre Frontend-URL eintragen.
app.use(cors({
  origin: '*'
}));
...
app.post('/api/sync', async (req, res) => {
  // ... (keep the same sync logic from the previous response)
  ...
  try {
      ...
      res.status(200).json({ status: 'success', message: 'Sync completed successfully.', logs: syncLogs });
  } catch (error) {
      console.error('An error occurred during the sync process:', error);
      addLog(`Error: ${error.message}`, 'error');
      res.status(500).json({ status: 'error', message: 'An internal server error occurred.', logs: syncLogs });
  }
});

// This is important for Vercel to correctly handle the server
module.exports = app;